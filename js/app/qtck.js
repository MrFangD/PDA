			...
			if (deletecheck == true) {
				mui.confirm("当前数据未保存，是否继续？","",btnArray,function(event){
					if (event.index == 0) {
						...
						document.getElementById('title').innerHTML = '其他出库单';
...
mui.plusReady(function(){
	...
	cklist.addEventListener("tap", function(e) {
		...
		if (tagClass && tagClass.indexOf("mui-table-view-cell") != -1) {
			...
			plus.storage.setItem('qtck_ckbh', e.target.id);
			mui('#popover1').popover('toggle',document.getElementById("showCkPicker"));
		}
	});
//	//获取缴库部门信息
...
//		var dwCheck = document.getElementById('showdwPicker').value;
			if (ckCheck == '') {
				alert('请先维护仓库信息！');
			} else if(plus.storage.getItem('qt_bh') == null) {
				alert('请先维护缴库部门信息！');
			} else if(plus.storage.getItem('qtckywlb_bh') == null) {
				alert('请先维护业务类别！');  
			} else if(plus.storage.getItem('qtckry_bh') == null) {
				alert('请先维护缴库人员信息！');
			} else {
				//根据基本信息生成单据编号
				url = plus.storage.getItem('url') + '/Handler/QueryHandler.ashx';
			...
			if (bluLists.checked ==true) {
				var arr = {
						'arrMain':[{
							"KCCKD1_PJLX":"Q",
							"KCCKD1_LSBH":"",
							"KCCKD1_DJRQ": document.getElementById("ywdate").value,
							"KCCKD1_YWRQ": document.getElementById("ywdate").value,
							"KCCKD1_SJDH": document.getElementById("sjdh").value,
							"KCCKD1_CKBH": plus.storage.getItem('qtck_ckbh'),
							"KCCKD1_DWBH": "",
							"KCCKD1_BMBH":plus.storage.getItem('qt_bh'),
							"KCCKD1_ZGBH":plus.storage.getItem('qtckry_bh'),
							"KCCKD1_YWBS":Djlx,
							"KCCKD1_LYBH":document.getElementById("djbh").value,
							"KCCKD1_LRXM":plus.storage.getItem('qtckry_bh'),
							"KCCKD1_BZ":"",
							"KCCKD1_LBBH":plus.storage.getItem('qtckywlb_bh'),
