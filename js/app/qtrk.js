			...
			if (deletecheck == true) {
				mui.confirm("当前数据未保存，是否继续？","",btnArray,function(event){
					if (event.index == 0) {
						...
						document.getElementById('title').innerHTML = '其他入库单';
...
mui.plusReady(function(){
	...
	cklist.addEventListener("tap", function(e) {
		...
		if (tagClass && tagClass.indexOf("mui-table-view-cell") != -1) {
			...
			plus.storage.setItem('qtrk_ckbh', e.target.id);
			mui('#popover1').popover('toggle',document.getElementById("showCkPicker"));
		}
	});
	//获取缴库部门信息
	...
	document.getElementById('search').addEventListener('tap',function(){
		...
		var ckCheck = document.getElementById('showCkPicker').value;
			if (ckCheck == '') {
				...
				alert('请先维护缴库部门信息！');
			} else if(plus.storage.getItem('qtrkywlb_bh') == null) {
				alert('请先维护业务类别！');
			} else if(plus.storage.getItem('qtjkry_bh') == null) {
				alert('请先维护缴库人信息！');
			} else {
				//根据基本信息生成单据编号
				url = plus.storage.getItem('url') + '/Handler/QueryHandler.ashx';
...
//          	break;
	...
	document.getElementById('hw').addEventListener('tap',function(){
		...
		mui.ajax(url,{
			data:{
				'action' :'queryhw',
				'ckbh' : plus.storage.getItem('qtrk_ckbh')
			},
			type:'post',//HTTP请求类型
			timeout:10000,//超时时间设置为10秒；
			success:function(data){
				...
				if (bluLists.checked == true) {
					var arr = {
						'arrMain':[{
							...
							"KCRKD1_CKBH": plus.storage.getItem('qtrk_ckbh'),
							"KCRKD1_DWBH": '',
							"KCRKD1_BMBH":plus.storage.getItem('jkbu_bh'),
							"KCRKD1_ZGBH":plus.storage.getItem('qtjkry_bh'),
							"KCRKD1_YWBS":Djlx,
							"KCRKD1_LYBH":'',
