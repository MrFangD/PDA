			...
			if (deletecheck == true) {
				mui.confirm("当前数据未保存，是否继续？","",btnArray,function(event){
					if (event.index == 0) {
						...
						document.getElementById('title').innerHTML = '销售出库单';
...
mui.plusReady(function(){
	...
	var ckbh;
	var canz;
	plus.storage.setItem('xsbm_bh','');
	plus.storage.setItem('xsy_bh', '');
	//返回主界面处理列表缓存数据
	...
	document.getElementById('s_pop1').addEventListener('input',function(){
		...
		if (filter == "") {
			...
			ck_pop = 'true';
			createCk(filter_data);
		}
	});
	function createCk(data){
		var str='' ;
	...
	var bmdata = [];
	var bm_pop = "";
	var cgbmButton = document.getElementById("xsbm");
	cgbmButton.addEventListener('tap', function(e){
		var zgbh ="";
		zgbh = plus.storage.getItem('xsy_bh');
		if (zgbh == null) {
			zgbh = "";
		} else{
			zgbh = plus.storage.getItem('xsy_bh');
	...
	function ListCreatBm(data){
		var str='' ;
		document.getElementById('xsbmlist').innerHTML = '';
		for (var i=0;i<data.length;i++) {
			str += '<li class="mui-table-view-cell" id="'+data[i].BMBH+'">'+
               data[i].BMBH +'：'+ data[i].BMMC+
        	'</li>'
		}
		document.getElementById('xsbmlist').innerHTML += str;
		if (bm_pop == '') {
			mui('#popover3').popover('toggle',document.getElementById("xsbm"));
		}
		bm_pop = '';
		mui('.mui-scroll-wrapper').scroll()
	};
	//点击列表选择
	var ul_city = document.getElementById('xsbmlist');
	ul_city.addEventListener("tap", function(e) {
		var tagClass = e.target.getAttribute("class");
		if (tagClass && tagClass.indexOf("mui-table-view-cell") != -1) {
			var select = e.target.innerText;
			document.getElementById("xsbm").value = select;
			arr = select.split('：')
			plus.storage.setItem('xsbm',arr[1]);
			plus.storage.setItem('xsbm_bh',e.target.id);
			mui('#popover3').popover('toggle',document.getElementById("xsbm"));
		}
	});
	...
	var rydata = [];
	var ry_pop = "";
	var ryxxButton = document.getElementById('xsy');
	ryxxButton.addEventListener('tap', function(e){
		var bmbh = '';
		bmbh = plus.storage.getItem('xsbm_bh');
		if (bmbh == null) {
			bmbh = ''
		} else{
			bmbh = plus.storage.getItem('xsbm_bh');
	...
	function ListCreatRy(data){
		var str='' ;
		document.getElementById('xsylist').innerHTML = '';
		for (var i=0;i<data.length;i++) {
			str += '<li class="mui-table-view-cell" id="'+data[i].ZGBH+'">'+
               data[i].ZGBH +'：'+ data[i].ZGXM+
        	'</li>'
			}
		document.getElementById('xsylist').innerHTML += str; 
		if (ry_pop == '') {
			mui('#popover4').popover('toggle',document.getElementById("xsy"));
		}
		ry_pop = '';
		mui('.mui-scroll-wrapper').scroll()
	};
	//点击列表选择
	var ul_city = document.getElementById('xsylist');
	ul_city.addEventListener("tap", function(e) {
		var tagClass = e.target.getAttribute("class");
		if (tagClass && tagClass.indexOf("mui-table-view-cell") != -1) {
			var select = e.target.innerText;
			document.getElementById("xsy").value = select;
			plus.storage.setItem('xsy_bh', e.target.id);
			mui('#popover4').popover('toggle',document.getElementById("xsy"));
		}
	});
	//往来单位
	var wldwdata = [];
	var wldw_pop = "";
	document.getElementById('khmc').addEventListener('tap', function(){
	...
	document.getElementById('s_pop').addEventListener('input',function(){
		...
		if (filter == "") {
			...
			wldw_pop = 'true';
			ListCreatDw(filter_data);
		}
	});
	function ListCreatDw(data){
		var str='' ;
		...
		if (wldw_pop == '') {
			mui('#popover').popover('toggle',document.getElementById("khmc"));
		}
		wldw_pop = '';
		mui('.mui-scroll-wrapper').scroll()
	};
	//点击列表选择
		...
		ul_city.addEventListener("tap", function(e) {
			...
			if (tagClass && tagClass.indexOf("mui-table-view-cell") != -1) {
				var select = e.target.innerText;
				document.getElementById("khmc").value = select;
	...
	document.getElementById('search').addEventListener('tap',function(){
		...
		var dwCheck = document.getElementById('khmc').value;
			if (ckCheck == '') {
				alert('请先维护仓库信息！');
			} else if(dwCheck == '') {
				alert('请先维护缴库部门信息！');
			} else if(plus.storage.getItem('xsckywlb_bh') == null) {
				alert('请先维护类别信息！');
			}else {
				//根据基本信息生成单据编号
				url = plus.storage.getItem('url') + '/Handler/QueryHandler.ashx';
...
//          	break;
	...
	function pcselect(data){
		...
		mui('#popover5').popover('toggle',document.getElementById("pch"));
		mui('.mui-scroll-wrapper').scroll();
	};
	//点击列表选择
	...
	function savemx(){
		...
		if (plus.storage.getItem('jldw') == null) {
			...
			jldwbh = plus.storage.getItem('jldw');
		}
		...
		if (document.getElementById("wlbh").value == '') {
			...
			if (canz == 1) {
				for (var i=0; i<smdata.length;i++) {
					if (document.getElementById("wlbh").value == smdata[i].F_wlbh && smdata[i].F_ggxh == document.getElementById('ggxh').value && document.getElementById("pch").value == smdata[i].F_pch && document.getElementById("hw").value == smdata[i].F_hwbh && document.getElementById("cgsmtm").value == smdata[i].F_smtm) {
						smdata[i].F_smsl = sl;
			...
			console.log(JSON.stringify(arrMxdata));
			returndata();
		}
	};
	//扫描界面删除
			...
			if (bluLists.checked == true) {
				var arr = {
					'arrMain':[{
						...
						"KCCKD1_BMBH":plus.storage.getItem('xsbm_bh'),
						"KCCKD1_ZGBH":plus.storage.getItem('xsy_bh'),
						"KCCKD1_YWBS":Djlx,
						"KCCKD1_LYBH":document.getElementById("djbh").value,
						"KCCKD1_LRXM":plus.storage.getItem('xsy_bh'),
						"KCCKD1_BZ":"",
						"KCCKD1_LBBH":plus.storage.getItem('xsckywlb_bh'),
						"KCCKD1_YTBH":'',//plus.storage.getItem('xsckyt_bh')
						"KCCKD1_CPSL":"",
						"KCCKD1_LYR":plus.storage.getItem('xsy_bh'),
						"KCCKD1_DWMC":plus.storage.getItem('xsbm'),
						"KCCKD1_LYBZ":"",
						"KCCKD1_HDBZ":"0"
					}],
					'arrMx':arrMxdata,
				...
				var arr = {
					'arrMain':[{
						...
						"KCCKD1_YWRQ": document.getElementById("ywdate").value,
						"KCCKD1_SJDH": document.getElementById("sjdh").value,
						"KCCKD1_CKBH": plus.storage.getItem('ck_bh'),
						"KCCKD1_DWBH": plus.storage.getItem('wldw_bh'),
						"KCCKD1_BMBH":plus.storage.getItem('xsbm_bh'),
						"KCCKD1_ZGBH":plus.storage.getItem('xsy_bh'),
						"KCCKD1_YWBS":Djlx,
