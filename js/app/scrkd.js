			...
			if (deletecheck == true) {
				mui.confirm("当前数据未保存，是否继续？","",btnArray,function(event){
					if (event.index == 0) {
						...
						document.getElementById('title').innerHTML = '生产入库单';
...
mui.plusReady(function(){
	...
	document.getElementById('s_pop1').addEventListener('input',function(){
		...
		if (filter == "") {
			...
			ck_pop = 'true';
			ListCreatyeCk(filter_data);
		}
	});
	function ListCreatyeCk(data){
		var str='' ;
...
//		var dwCheck = document.getElementById('showdwPicker').value;
			if (ckCheck == '') {
				...
				alert('请先维护缴库部门信息！');
			} else if(plus.storage.getItem('scywlb_bh') == null) {
				alert('请先维护业务类别！');
			} else if(plus.storage.getItem('jkry_bh') == null) {
				alert('请先维护缴库人员信息！');
			} else {
				//根据基本信息生成单据编号
				url = plus.storage.getItem('url') + '/Handler/QueryHandler.ashx';
...
//          	break;
		...
		mui.ajax(url,{
			...
			success:function(data){
				...
				if (data.length == 0) {
					...
					plus.storage.setItem('LSWLZD_JLDW', data[0].LSWLZD_JLDW)
	...
	function savemx(){
		...
		var jldwbh;
		if (plus.storage.getItem('LSWLZD_JLDW') == null) {
			jldwbh = '';
		} else{
			jldwbh = plus.storage.getItem('LSWLZD_JLDW');
		}
	...
	if (document.getElementById("wlbh").value == '') {
		...
		if (canz == 1) {
			for (var i=0; i<smdata.length;i++) {
				if (document.getElementById("wlbh").value == smdata[i].F_wlbh  && smdata[i].F_ggxh == document.getElementById('ggxh').value  ) {
					smdata[i].F_smsl = sl;
			...
			for (var i=0; i<smdata.length;i++) {
				if (document.getElementById("wlbh").value == smdata[i].F_wlbh && smdata[i].F_pch == document.getElementById('pch').value && smdata[i].F_ggxh == document.getElementById('ggxh').value && document.getElementById("cgtm").value ==  smdata[i].F_tmh ) {
	...
	saveButton.addEventListener('tap', function(event){
		...
		if (a == '') {
			...
			if (bluLists.checked == true) {
				var arr = {
					'arrMain':[{
						...
						"KCRKD1_CKBH": plus.storage.getItem('scrk_ckbh'),
						"KCRKD1_DWBH": "",
						"KCRKD1_BMBH":plus.storage.getItem('jkbu_bh'),
						"KCRKD1_ZGBH":plus.storage.getItem('jkry_bh'),
						"KCRKD1_YWBS":Djlx,
				...
				var arr = {
					'arrMain':[{
						...
						"KCRKD1_YWRQ": document.getElementById("ywdate").value,
						"KCRKD1_SJDH": document.getElementById("sjdh").value,
						"KCRKD1_CKBH": plus.storage.setItem('scrk_ckbh'),
						"KCRKD1_DWBH": "",
						"KCRKD1_BMBH":plus.storage.getItem('jkbu_bh'),
						"KCRKD1_ZGBH":plus.storage.getItem('jkry_bh'),
						"KCRKD1_YWBS":Djlx,
			...
			url = plus.storage.getItem("url") + '/Handler/KcrkdsaveHandler.ashx';
